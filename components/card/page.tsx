...
export default async function CardComponent() {
  ...
  console.log(blogs);
  const formattedDate = new Date(blogs[0].date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
        ...
        {blogs.map((blog: any) => (
          <div key={blog.slug} className={styles.card}>
            <div className={styles.cardHeader}>
              <Image
                src={blog.featuredImage?.node?.sourceUrl || "/rendering-platform-featured.jpg"}
            ...
            <div className={styles.cardAuthor}>
              <p  className={styles.cardDateText}>By <span className={styles.authorNameText}>{"Position2"}</span></p>
            </div>
